...
### Ad Integration Architecture (v2.1+)

#### Overview

SecureVault v2.1+ integrates Google AdMob native ads to sustain development while maintaining strict separation between password data and ad functionality.

**Critical Security Principle**: Password encryption and storage are **completely isolated** from ad SDK. No password data is ever accessible to AdMob.
...
│                     UI Layer (MainScreen)                   │
│  ┌─────────────────────┐        ┌──────────────────────┐   │
│  │   Password Items    │        │   NativeAdCard       │   │
│  │   (Encrypted Data)  │  ❌ NO │   (AdMob SDK)        │   │
│  │                     │  SHARING│                      │   │
│  │   • PasswordItem    │        │   • Ad headline      │   │
│  │   • Compose cards   │        │   • Ad body          │   │
│  │                     │        │   • Call to action   │   │
│  └─────────────────────┘        └──────────────────────┘   │
└─────────────────────────────────────────────────────────────┘
                  ▼                           ▼
...
│  • Password StateFlow       │   │  • Ad cache (3 ads)     │
│  • Encryption via Repo      │   │  • MobileAds SDK        │
│  • NO ad access             │   │  • Test/prod ad units   │
└─────────────────────────────┘   └─────────────────────────┘
                  ▼                           ▼
┌─────────────────────────────┐   ┌─────────────────────────┐
│   PasswordRepositoryImpl    │   │   AdMob Network         │
│  • SecurityManager          │   │  • Device ad ID         │
│  • Keystore encryption      │   │  • Impressions          │
│  • Database operations      │   │  • NO password access   │
└─────────────────────────────┘   └─────────────────────────┘
```
...
##### 1. AdManager.kt
**Location**: `app/src/main/java/com/securevault/utils/AdManager.kt`
**Purpose**: Manages AdMob SDK lifecycle and ad caching
**Features**:
- Lazy initialization (called from MainActivity.onCreate())
- Pre-loads 3 native ads for smooth scrolling
- Test ad units in debug builds (`ca-app-pub-3940256099942544/2247696110`)
- Production ad units in release builds (configure in AdManager.kt)
- StateFlow-based reactive ad state
...
class AdManager(context: Context) {
    ...
    // Load ads into cache (automatic reload when low)
    fun loadAds()

    // Get ad from cache
    fun getAd(): NativeAd?

    // Clean up ads (call from ViewModel.onCleared())
    fun cleanup()

    // Reactive state
    val nativeAds: StateFlow<List<NativeAd>>
...
**Security Notes**:
- AdManager has NO access to password data
- Operates in separate coroutine scope
- No shared memory with password operations

##### 2. NativeAdCard.kt
**Location**: `app/src/main/java/com/securevault/ui/components/NativeAdCard.kt`
**Purpose**: Material 3 styled native ad composable
**Features**:
- Matches password card design (Card elevation, padding)
- Distinct surfaceVariant color for visual separation
- "Ad" or "Sponsored" label for transparency
- Responsive layout with headline, body, call-to-action
- AndroidView wrapper for AdMob's NativeAdView

**Two Variants**:
```kotlin
// Full AndroidView implementation
@Composable fun NativeAdCard(nativeAd: NativeAd)

// Compose-native variant
@Composable fun NativeAdCardCompose(nativeAd: NativeAd)
```

**Privacy Transparency**:
- Clear "Ad" label with info icon
- AdChoices icon (automatic via AdMob)
- No deceptive styling

##### 3. MainScreen.kt Modifications
**Changes**: Updated PasswordList composable to interleave ads

**Ad Frequency Logic**:
```kotlin
- No ads if password count < 5
- One ad at position 5 if password count 5-14
- Ads every 8 items if password count >= 15

Examples:
- 3 passwords: [P, P, P] (no ads)
- 7 passwords: [P, P, P, P, P, Ad, P, P] (ad at position 5)
- 20 passwords: [P×8, Ad, P×8, Ad, P×4] (ads at positions 8, 16)
```
    ...
    LazyColumn {
        items(count = totalItems) { index ->
            if (adPositions.contains(index)) {
                NativeAdCardCompose(nativeAd = ads[...])
            } else {
                PasswordItem(password = passwords[...], onClick = ...)
            }
        }
    }
}
...
- Ads pre-loaded in background (no UI blocking)
- LazyColumn recycling works with mixed item types
- Key-based item identification for stable scrolling

##### 4. MainViewModel.kt Integration
**Changes**:
- Add AdManager dependency
- Expose nativeAds StateFlow
- Cleanup ads in onCleared()
...
class MainViewModel(application: Application) : AndroidViewModel(application) {
    ...
    override fun onCleared() {
        super.onCleared()
        adManager.cleanup() // Prevent memory leaks
    }
}
```
...
**Ad Display**:
- [ ] Ads load in debug build with test ad units
- [ ] No ads shown for < 5 passwords
- [ ] One ad at position 5 for 5-14 passwords
- [ ] Ads every 8 items for 15+ passwords
- [ ] Ad card styling matches Material 3 theme
- [ ] "Ad" label visible on all ad cards
- [ ] AdChoices icon present

**Performance**:
- [ ] LazyColumn scroll remains smooth (60 FPS)
- [ ] No jank when ads appear/disappear
- [ ] Memory usage stable (check Android Studio Profiler)
- [ ] Ads load in background without blocking UI

**Privacy Verification**:
- [ ] Network traffic only to AdMob endpoints
- [ ] NO password data in network logs
- [ ] Encrypted passwords remain in database
...
adb shell dumpsys package com.securevault.debug | grep -i "network"

# Check AdMob logs
adb logcat | grep -E "(AdManager|AdMob|Ads)"

# Verify encrypted passwords (should NOT see plaintext)
adb shell "run-as com.securevault.debug cat databases/password_database" | strings
```
...
# Open app, navigate password list
# Verify in proxy logs:
1. Ad requests go to: *.googlesyndication.com, *.doubleclick.net
2. Request payloads contain: device ID, app ID, ad unit ID
3. NO password data in ANY network request
4. NO encryption keys in network traffic
```

**Expected Network Endpoints**:
- ✅ `googlesyndication.com` (ad serving)
- ✅ `doubleclick.net` (ad tracking)
- ✅ `google.com/ads` (ad SDK)
- ❌ NO other external endpoints

#### Privacy & Security Guarantees

**What AdMob Can Access**:
- Device advertising ID (resettable)
- App package name
- Android version
- Device model/manufacturer
- Screen size, language, timezone
- Ad unit ID, impression events

**What AdMob CANNOT Access**:
- ❌ Password database (encrypted in Room)
- ❌ Encryption keys (hardware Keystore)
- ❌ Biometric data
- ❌ User passwords, usernames, notes
- ❌ Backup files
- ❌ Any data from SecurityManager

**Architectural Guarantees**:
1. **Separate Initialization**: AdMob SDK initializes AFTER migration check
2. **No Shared State**: Password StateFlow and Ad StateFlow are separate
3. **Different Coroutine Scopes**: Password ops and ad loading isolated
4. **UI Component Isolation**: PasswordItem and NativeAdCard are separate composables
5. **No Database Access**: AdManager has NO reference to PasswordRepository

#### Compliance

**GDPR & Privacy Laws**:
- ✅ Privacy policy updated (README.md)
- ✅ Third-party services disclosed (LICENSE)
- ✅ User control over ad personalization (Android Settings)
- ✅ Clear ad labeling for transparency
- ✅ No deceptive practices

**Google Play Policy**:
- ✅ Ads clearly labeled ("Ad" badge)
- ✅ No interstitials on app launch
- ✅ AdChoices icon present
- ✅ Privacy policy linked in app listing
...
**Solution**: Check logcat for AdMob errors, verify internet permission in manifest, ensure AdMob SDK initialized

**Issue**: "Invalid ad unit ID" error
**Solution**: Using debug build? Should use test ad unit ID. In release, replace PRODUCTION_AD_UNIT_ID in AdManager.kt

**Issue**: Memory leaks
**Solution**: Verify AdManager.cleanup() called in MainViewModel.onCleared()

**Issue**: Network traffic shows password data
**Solution**: **CRITICAL SECURITY BUG** - Report immediately, this should NEVER happen

**Debug Logging**:
```bash
# Enable verbose AdMob logs
adb shell setprop log.tag.Ads VERBOSE
adb logcat | grep Ads

# Check ad cache state
adb logcat | grep "Ad cache:"
...
**Planned Features**:
- [ ] User preference to reduce ad frequency
- [ ] Ad-free premium version (in-app purchase)
- [ ] More granular ad controls in settings
- [ ] Revenue transparency dashboard

**Performance Optimizations**:
- [ ] Increase ad cache size based on list length
- [ ] Lazy ad loading (only when scrolling near position)
- [ ] Ad preloading prediction based on scroll velocity
...
- **Google AdMob**: Native ads for revenue (`com.google.android.gms:play-services-ads:22.6.0`)
